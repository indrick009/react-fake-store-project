...
export default function Cart({ items, onClose, onRemove, onUpdateQty }: CartProps) {
    ...
    <>
      ...
      <aside className="fixed right-0 top-0 h-full w-full max-w-sm bg-white z-50 shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
          <h2 className="font-display text-xl font-bold text-stone-900">Mon Panier</h2>
          <button
            onClick={onClose}
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-3 p-3 bg-stone-50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-body text-xs text-stone-700 line-clamp-2 font-medium">{item.title}</p>
                  <p className="font-display text-sm font-bold text-stone-900 mt-1">${item.price.toFixed(2)}</p>
